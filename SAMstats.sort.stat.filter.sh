...
if [ -z $(samtools view -H $inputf | grep "SO" | grep "queryname") ]
then 
    echo "Sorting by read name!" 
    samtools sort -n  --threads $threads  -o $inputf.sorted $inputf
    ...
    if [-z $inputf.bai ]
    then
	echo "Generating index for input file" 
	samtools index $inputf
    fi
    $inputf.sorted=$inputf 
    $inputf.sorted.bai=$inputf.bai
fi
...
withMateMappedToDiffChrom_qcPassed=`samtools view -F 0x200 -f 0x1 -F 0x4 -F 0x8 $inputf.sorted |awk '{ if ($7 == '=') { print } }'| cut -f1 | uniq | wc -l `
echo "withMateMappedToDiffChrom_qcPassed:$withMateMappedToDiffChrom_qcPassed"
withMateMappedToDiffChrom_qcFailed=`samtools view -f 0x200 -f 0x1 -F 0x4 -F 0x8 $inputf.sorted |awk '{ if ($7 == '=') { print } }'| cut -f1 | uniq | wc -l `
...
withMateMappedToDifferentChromQC5_qcPassed=`samtools view -F 0x200 -F 0x4 -q5 $inputf.sorted | awk '{ if ($7 == '=') { print } }'|cut -f1 | uniq | wc -l `
echo "withMateMappedToDifferentChromQC5_qcPassed:$withMateMappedToDifferentChromQC5_qcPassed"
withMateMappedToDifferentChromQC5_qcFailed=`samtools view -f 0x200 -F 0x4 -q5 $inputf.sorted | awk '{ if ($7 == '=') { print } }'|cut -f1 | uniq | wc -l `
...
percent_mapped_qcPassed=$(( mapped_qcPassed / qcPassed ))
percent_mapped_qcFailed=$(( mapped_qcFailed / qcFailed ))
percent_properlyPaired_qcPassed=$(( properlyPaired_qcPassed / qcPassed ))
percent_properlyPaired_qcFailed=$(( properlyPaired_qcFailed / qcFailed))
percent_singletons_qcPassed=$(( singletons_qcPassed / qcPassed ))
percent_singletons_qcFailed=$(( singletons_qcFailed / qcFailed ))
