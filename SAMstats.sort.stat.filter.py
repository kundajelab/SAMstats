...
def parse_args(): 
    ...
    parser.add_argument("--chunk_size",type=int,default=500,help="Number of lines to read a time from sortedSamFile")
    parser.add_argument("--filter_before_stat",action="store_true",default=False,help="Should input be filtered for unique read names before mapping stats are computed (default is to filter after stats are computed") 
...
def add_read_stats(flag,mapq,rnext,cur_flagstat): 
    ...
    '''
    temp_flagstat=np.zeros(14,dtype=np.uint64) 
    ...
    if qc_passed==True:
        cur_flagstat[0]=np.bitwise_or(cur_flagstat[0],temp_flagstat) 
    else: 
        cur_flagstat[1]=np.bitwise_or(cur_flagstat[1],temp_flagstat)
    return cur_flagstat 
...
def initialize_flagstat(stat): 
    ...
    '''
    qc_passed=np.zeros(14,dtype=np.uint64) 
    qc_failed=np.zeros(14,dtype=np.uint64) 
...
def update_flagstat_for_readname(global_flagstat, cur_flagstat): 
    ...
    '''
    global_flagstat[0]+=cur_flagstat[0] 
    ...
    for chunk in pd.read_csv(sam, header=None, chunksize=args.chunk_size,delim_whitespace=True,comment='@',usecols=[0,1,4,6,9],engine='c'):
        for index,line in chunk.iterrows(): 
            ...
            if (args.filter_before_stat is True) and (new_seqid == cur_seqid):
                continue 
                #we have already calculated statistics for this read, this is a duplicate alignment and we skip it 
